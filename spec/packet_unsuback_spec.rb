...
describe MqttRails::Packet::Unsuback do
  context "Create a simple suback packet" do
    it "Successfully create suback packet from header" do
      packet = MqttRails::Packet::Base.create_from_header(0xB0)
      expect(packet.inspect).to eq("#<MqttRails::Packet::Unsuback: 0x00>")
    end
  end

  context "Encode body for suback packet" do
    packet = MqttRails::Packet::Unsuback.new
  ...
  context "Parse body of suback packet" do
   ...
   context "Raising execption" do
     it "Fail to encode body for empty topics" do
       ...
       expect { packet.parse_body(body) }.to raise_error(MqttRails::PacketFormatException, "Extra bytes at end of Unsubscribe Acknowledgment packet")
     end
   end
end
